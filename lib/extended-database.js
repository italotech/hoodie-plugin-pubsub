...
exports.ExtendedDatabaseAPI = function ExtendedDatabaseAPI(hoodie, db) {
  ...
  function designDocAdd(ddoc_type, ddoc_name, ddoc_data, callback2) {
    ...
    hoodie.request('GET', ddoc_url, {}, function (err, ddoc, res) {
      ...
      hoodie.request('PUT', ddoc_url, {data: ddoc}, callback2);
    });
  }

  function designDocRemove(ddoc_type, ddoc_name, callback2) {
  ...
  db.addFilter = function (name, filterFunction, callback2) {
    designDocAdd('filters', name, filterFunction, callback2);
  };
  ...
  db.removeFilter = function (name, callback2) {
    designDocRemove('filters', name, callback2);
  };
  ...
  db.addIndex = function (name, mapReduce, callback2) {
    designDocAdd('views', name, mapReduce, callback2);
  };
  ...
  db.removeIndex = function (name, callback2) {
    designDocRemove('views', name, callback2);
  };
...
/**
  ...
  db.query = function (index, params, callback2) {
    // `params` is optional, when only two args passed second is callback2.
    ...
    hoodie.request('GET', view_url, params, function (err, data) {
      if (err) {
        return callback2(err);
      }
